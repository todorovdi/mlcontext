        ...
        for freq_name in freqs:
            ...
            for ii, analysis in enumerate(analyses):
                ...
                sns.violinplot(x='Type', y='Decoding Performance', hue='Condition',
                               ...
                               linewidth=0.5, ax=axs[ii])
                axs[ii].axhline(y=0, linestyle='dotted', color='k', linewidth=0.8)
                if p_stable < 0.05:
                    axs[ii].text(-0.4, -0.1, f'p={p_stable:.3f}', fontsize=7, color='red')
                else:
                    axs[ii].text(-0.4, -0.1, f'p={p_stable:.3f}', fontsize=7)
                axs[ii].text(-0.4, -0.11, f'm={m_stable:.3f}', fontsize=7)
                if p_random < 0.05:
                    axs[ii].text(0.1, -0.1, f'p={p_random:.3f}', fontsize=7, color='red')
                else:
                    axs[ii].text(0.1, -0.1, f'p={p_random:.3f}', fontsize=7)
                axs[ii].text(0.1, -0.11, f'm={m_random:.3f}', fontsize=7)
                if p_diff < 0.05:
                    axs[ii].text(0, 0.3, f'p={p_diff:.3f}', fontsize=7, color='red')
                else:
                    axs[ii].text(0, 0.3, f'p={p_diff:.3f}', fontsize=7)
                axs[ii].set_ylim(-0.2, 0.4)
                axs[ii].yaxis.set_label_text('Decoding Performance (r)')
                axs[ii].xaxis.set_label_text('')
                axs[ii].get_legend().set_visible(False)
                axs[ii].set_title('Decoding %s' % analysis)
                plt.tight_layout()
            fname_fig = op.join(path_fig,save_folder,
                f'{rt}_{analysis_name}_{freq_name}_{control}') 
