...
class LocaError:
    ...
    def initialize_parameters(self):
        ...
        self.add_param_comment('# Time inside the home position before trial starts (seconds)')
        self.add_param('time_inside', 0.5)
        self.add_param_comment('# Time for feedback (seconds)')
        self.add_param('time_feedback', 0.25)
        self.add_param_comment('# Time for target_phase (seconds)')
        self.add_param('time_target_phase', 0.5)
        self.add_param_comment('# Time for intertrial interval (seconds)')
        self.add_param('time_ITI', 1.5)
        self.add_param_comment('# Jitter time (max) to add to ITI (seconds) ')
    ...
    def __init__(self, subject_name = '', task_id = '', session_id=''):
        ...
        self.currentText = self.myfont.render('Hits: 0/0', True, (255, 255, 255))
        self.instructions_text = self.myfont.render(self.string_instructions,
                                                    True, (255, 255, 255))
        self.start_task = 0
        self.color_photodiode = [0, 0, 0]
        self.home_position = (int(round(self.params['width']/2.0)), int(round(self.params['height']/2.0)))
        self.target_position = (int(self.home_position[0] + np.random.uniform(-4, 4)),
                                int(self.home_position[1] + np.random.uniform(-4, 4)))
        ...
        nbTrials = 10  # Number of trials per target (4 targets)
        x = np.zeros(nbTrials)
        x1 = np.concatenate((x, x + 1, x + 2, x + 3))
        #  Error size vector (normal distribution zero-centered with sigma=10deg)
        x2 = np.concatenate((np.random.normal(0, 13, nbTrials),
                             np.random.normal(0, 13, nbTrials),
                             np.random.normal(0, 13, nbTrials),
                             np.random.normal(0, 13, nbTrials)))
        self.trialsArray = np.random.permutation(np.column_stack((x1, x2)))
    ...
    def init_target_positions(self):
        ...
        for x in range(0, len(targetAngs)):
            ...
            self.target_types.append((int(round(self.params['width']/2.0 + np.cos(current) * self.params['radius'])),
                                     int(round(self.params['height']/2.0 + np.sin(current) * self.params['radius']))))
        ...
        if (self.start_task):
            ...
            if self.current_phase == self.REST_PHASE:
                ...
                pygame.draw.circle(self._display_surf, [0, 0, 0],
                                   self.home_position, int(self.params['radius_cursor']*2.0), 2)
            ...
            if (self.current_phase == self.FEEDBACK_PHASE):
                pygame.draw.circle(self._display_surf, [255, 255, 255],
                                   ...
                                   self.params['radius_cursor'], 0)
            pygame.draw.rect(self._display_surf, self.color_photodiode, (0, 0, self.params['width'], 30), 0)
    ...
    def project_coordinates(self, coordinates, perturbation):
        ...
        my_coords = [-1, -1]
        coordinates = self.target_types[self.target_to_show]

        my_coords[0] = coordinates[0] - self.params['width']/2.0
        my_coords[1] = coordinates[1] - self.params['height']/2.0

        cursorReachX = np.cos(np.arctan2(my_coords[1], my_coords[0]) + perturbAngle) * self.params['radius']
        cursorReachY = np.sin(np.arctan2(my_coords[1], my_coords[0]) + perturbAngle) * self.params['radius']
    ...
    def on_loop(self):
        if (self.current_phase == self.REST_PHASE):
            ...
            self.counter_target_phase = self.counter_target_phase+1
            if (self.counter_target_phase == self.params['FPS'] * self.params['time_target_phase']):
    ...
    def update_log(self):
        ...
        self.current_log.append(self.trialsArray[self.counter_trials, 1]) #error
        self.current_log.append(self.buttonpress)
